...
const create = asyncMiddleware(async (req, res) => {
  await roleService.ensureUnique({ name: req.body.name });
...
const getAll = asyncMiddleware(async (req, res) => {
  const data = await roleService.getAll({}, req.validated.query);
...
const updateById = asyncMiddleware(async (req, res) => {
  await roleService.ensureNotReferenced(req.params.id);

  await roleService.ensureUnique({ name: req.body.name }, req.params.id);
...
const deleteById = asyncMiddleware(async (req, res) => {
  ...
  const data = await roleService.updateById(req.params.id, update);
  if (!data) throwError(MESSAGES.DATA_NOT_FOUND, HTTP_STATUS.NOT_FOUND);
