...
const create = asyncMiddleware(async (req, res) => {
  await permissionService.ensureUnique({ name: req.body.name });
...
const getAll = asyncMiddleware(async (req, res) => {
  const data = await permissionService.getAll({}, req.validated.query);
...
const updateById = asyncMiddleware(async (req, res) => {
  await permissionService.ensureNotReferenced(req.params.id);

  await permissionService.ensureUnique({ name: req.body.name }, req.params.id);

  const data = await permissionService.updateById(req.params.id, req.body);
  if (!data) throwError(MESSAGES.DATA_NOT_FOUND, HTTP_STATUS.NOT_FOUND);
...
const deleteById = asyncMiddleware(async (req, res) => {
  ...
  const data = await permissionService.updateById(req.params.id, update);
  if (!data) throwError(MESSAGES.DATA_NOT_FOUND, HTTP_STATUS.NOT_FOUND);
